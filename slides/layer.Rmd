...
First, we can simply the level of educational attainment `SCHL` variable' by only including some of the factor levels and saving in a new data frame `filter_SCHL`.   

Here, we include only individuals where the highest level of educational attainment is high school (16), bachelor's degree (21), a masters degree (22), or a doctorate (24). {:.notes}

```{r, handout = 0}
filter_SCHL <- pums[pums$SCHL %in% c(16, 21, 22, 24),]
```

We can recode  `SCHL` from a numeric code to the highest education level as we want displayed. All data from the other levels will be converted to NA in `SCHL`. To remove the NA's from the data in the ggplot object, use `na.omit.`
...
perform quick data transformations while plotting. Instead of using `geom_point` to display all the data points, we can add points for the mean at each educational level.
...
To show the difference in wages based on sex, we can color the points based on the `SEX` data column. This time, color is specificied in the `aes` function because we are aesthetically mapping the data to an visual element in the plot.
...
and the visual elements, but they can still affect the output. For example, see what happens when we switching the order of the levels of `SEX` in the data and then replot.
